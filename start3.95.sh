...
# Version - 0.95 (Добавлен 7 пункт, есть проверка minecraftuser + по мелочи)
...
then
        echo "User exist" #> /dev/null #2>&1;
else
	useradd -s /bin/bash minecraftuser
        echo "Come up with a password for the user minecraftuser"
        passwd minecraftuser
	sudo mkdir -p /home/minecraftuser/minecraft/
	cp start.sh /home/minecraftuser/minecraft/start.sh
        sudo chown -R minecraftuser:minecraftuser /home/minecraftuser/minecraft/
        sudo chmod +wxr /home/minecraftuser/minecraft/start.sh
fi

if [[ -d /usr/lib/jvm/ ]]
then
        chown -R minecraftuser:minecraftuser /usr/lib/jvm/
...
alive=1
cd /home/minecraftuser/minecraft/
while [[ ${alive} -eq 1 ]]
do
echo -e "\e[92mHi, this a menu install your minecraft server, change\e[0m"
echo -e "\e[33m1 - install server and run, 2 - options server, 3 - only install, 4 - remove all files, 5 - exit, 6 - start server, 7 - default options server\e[0m"
read change
case ${change} in
1)
...
2)
        ...
        case ${options} in
                ...
                13)
                        echo "WIRNING. YOU WORLD WILL BE DELITED, IF YOU CHOOSE LEVEL TYPE, YOUR READY? 1 - Yes, 2 - NO"
                        read delw
                        if [[ ${delw} -eq 1 ]]
                        then
                                echo "Choose level type. 1 - DEFAULT, 2 - FLAT, 3 - LARGEBIOMES, 4 - AMPLIFIED"
                                read lety
                                case ${lety} in
                                1)
                                        sed -i "s/level-type=.*/level-type=DEFAULT/g" server.properties
                                        echo "Your level type is DEFAULT"
                                        ;;
                                2)
                                        sed -i "s/level-type=.*/level-type=FLAT/g" server.properties
                                        echo "Your level type is FLAT"
                                        ;;
                                3)
                                        sed -i "s/level-type=.*/level-type=LARGEBIOMES/g" server.properties
                                        echo "Your level type is LARGEBIOMES"
                                        ;;
                                4)
                                        sed -i "s/level-type=.*/level-type=AMPLIFIED/g" server.properties
                                        echo "Your level type is AMPLIFIED"
                                        ;;
                                esac
                        elif [[ ${delw} -eq 2 ]]
                        then
                ...
                14)
                        echo "hardcore? 1 - Yes, 2 - No"
                        read hard
                        if [[ ${hard} -eq 1 ]]
                        then
                                sed -i "s/hardcore=.*/hardcore=true/g" server.properties
                                echo "HAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARD ON"
                        elif [[ ${hard} -eq 2 ]]
                        then
                                sed -i "s/hardcore=.*/hardcore=false/g" server.properties
                                echo "Hard OFF :("
                        fi
                        ;;

                15)
                        echo "Enable command block? 1 - Yes, 2 - No"
                        read ecb
                        if [[ ${ecb} -eq 1 ]]
                        then
                                sed -i "s/enable-command-block=.*/enable-command-block=true/g" server.properties
                                echo "Enable block ON"
                        elif [[ ${ecb} -eq 2 ]]
                        then
                                sed -i "s/enable-command-block=.*/enable-command-block=false/g" server.properties
                                echo "Command block OFF"
                        fi
                        ;;
                16)
                ...
                17)
                        echo "No, no, you're not going to play with this :)"
                        ;;
                18)
                        echo "No, no, you're not going to play with this :)"
                        ;;
                ...
                19)
                        echo "Wtire max world size. 1 - 29999984"
                        read mws
                        if [[ ${mws} -ge 1 && ${mws} -le 29999984 ]]
                        then
                                sed -i "s/max-world-size=.*/max-world-size=${mws}/g" server.properties
                                echo "Your max world size ${mws}"
                        else
                                echo "Error.Write number 1 - 29999984"
                        fi
                        ;;
...
6)
        ...
        ;;
7)
	if [[ -e server.properties.bak ]]
	then
		rm server.properties
		mv server.properties.bak server.properties
		echo "Your settings have been returned to default"
	else
		echo "I dont see file 'server.properties.bak', maybe you dont make this"
	fi
	;;
esac
done
chown -R minecraftuser:minecraftuser /usr/lib/jvm/
chown -R minecraftuser:minecraftuser /home/minecraftuser/minecraft/
su - minecraftuser
cd /home/minecraftuser/minecraft
